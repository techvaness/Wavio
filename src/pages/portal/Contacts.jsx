        ...
        {tab === 'overview' && (
          <>
            ...
            <div className="space-y-2">
              <button className="w-full flex items-center justify-center gap-2 py-2.5 text-xs font-bold text-white bg-[#1a3fbf] hover:bg-[#2e5de6] rounded-xl transition-colors">
                <MessageSquare size={13} /> Start conversation
              </button>
              <button className="w-full py-2.5 text-xs font-semibold text-[#475569] border border-[#e4e7ed] hover:bg-[#f8fafc] rounded-xl transition-colors">Edit contact</button>
              <button onClick={() => contactApi.delete(contact.id).then(onClose).catch(console.error)}
                className="w-full py-2.5 text-xs font-semibold text-red-500 border border-red-200 bg-red-50 hover:bg-red-100 rounded-xl transition-colors">Delete contact</button>
            </div>
          </>
        )}
      ...
      <AnimatePresence>
        {selected && (
          <>
            ...
            <ContactDrawer contact={selected} onClose={() => setSel(null)} />
          </>
        )}
      </AnimatePresence>
    </div>
