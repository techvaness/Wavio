...
import { Search, MoreVertical, X, Users, MessageSquare, CreditCard, Loader2 } from 'lucide-react'
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        {activeTab === 'overview' && (
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              {[
                ['Agents', account.agent_count, Users],
                ['Conversations', account.convo_count, MessageSquare],
                ['Plan', account.plan, CreditCard],
                ['Created', account.created_at?.slice(0, 10), null],
              ].map(([label, val]) => (
                <div key={label} className="bg-[#f8fafc] rounded-xl p-3 border border-[#e4e7ed]">
                  <p className="text-[10px] text-[#94a3b8] mb-1">{label}</p>
                  <p className="text-base font-bold text-[#0f172a] capitalize">{val}</p>
                </div>
              ))}
            </div>
            <div>
        ...
        {activeTab === 'usage' && (
          <div className="space-y-3">
            {[
              { label: 'Conversations', used: account.convo_count, limit: account.plan === 'enterprise' ? 'Unlimited' : account.plan === 'pro' ? 5000 : 100 },
              { label: 'Agent seats', used: account.agent_count, limit: account.plan === 'enterprise' ? 'Unlimited' : account.plan === 'pro' ? 10 : 1 },
              ...
              return (
                <div key={label} className="bg-[#f8fafc] rounded-xl p-4 border border-[#e4e7ed]">
                  <div className="flex justify-between text-xs mb-2">
                    ...
                    <span className="font-bold text-[#0f172a]">{used} / {limit}</span>
                  </div>
...
export default function Accounts() {
  ...
  const filtered = rows.filter((a) => {
    ...
    const matchQ = !q || a.name.toLowerCase().includes(q) || (a.owner_email || '').toLowerCase().includes(q) || (a.owner_name || '').toLowerCase().includes(q)
  ...
  return (
    <div className="p-5 relative" style={{ fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div className="flex flex-wrap items-center gap-2 mb-4">
        ...
        {['All', 'Free', 'Pro', 'Enterprise', 'starter'].map((p) => (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#f8fafc] border-b border-[#e4e7ed]">
                {['Account', 'Owner', 'Plan', 'Agents', 'Conversations', 'Created', ''].map((h) => (
            ...
            <tbody className="divide-y divide-[#f8fafc]">
              {filtered.map((a, i) => (
                <motion.tr key={a.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }}
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full capitalize ${planColors[a.plan] ?? planColors.starter}`}>{a.plan}</span>
                  </td>
